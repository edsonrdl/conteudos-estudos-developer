...
A chave está na operação **AND bit a bit** no **4º octeto**. Vamos focar só nele:

---

### 🔍 Zoom no 4º Octeto

O IP é `150` e a máscara `/26` tem `192` no 4º octeto:
...
> A máscara age como um **"filtro"** — onde ela tem `0`, **apaga** o bit do IP. Onde tem `1`, **mantém**.

O resultado `10000000` = **128** → por isso `192.168.10.**128**`

---

### 📏 Como são definidos os Saltos?

O salto vem direto dos **bits de host**. No `/26` temos **6 bits de host**:
...
> O IP `150` cai no bloco que começa em `128` → por isso a rede é `.128`
...
Aí é só listar os múltiplos: `0, 64, 128, 192` → esses são sempre os **inícios de cada sub-rede**.
