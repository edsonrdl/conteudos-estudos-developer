...
### 4.3 Loop Engineering — Do Prompt ao Sistema Autônomo _(2026)_

> Termo que se popularizou em meados de 2026 para nomear a disciplina de projetar o **sistema que roda o agente**, em vez de digitar o próximo prompt manualmente a cada passo.

- **4.3.1. Definição e Motivação**
    
    - 4.3.1.1. **O que é:** Loop Engineering é a prática de construir um sistema de controle que dispara, verifica, tenta de novo e para um agente de IA automaticamente, substituindo a condução manual turno a turno. A frase-síntese do conceito: **"um loop é uma tarefa mais uma verificação — uma tarefa sem verificação é apenas esperança."**
    - 4.3.1.2. **Diferença de Prompt Engineering:** Prompt Engineering otimiza uma única entrada para uma única saída, exigindo julgamento humano depois de cada resposta. Loop Engineering automatiza o ciclo iterativo inteiro, embutindo as regras de julgamento no próprio sistema — a diferença entre "acertar um turno" e "completar a tarefa inteira, sem supervisão, ao longo de centenas de iterações".
    - 4.3.1.3. **Por que emergiu agora:** Os modelos cruzaram um limiar de confiabilidade para trabalho de longo horizonte. Quando um modelo consegue se recuperar dos próprios erros por horas seguidas, o gargalo deixa de ser "qualidade da resposta" e passa a ser "qualidade do design da orquestração" — e é exatamente esse design que o Loop Engineering nomeia e formaliza.
- **4.3.2. Os Quatro Componentes de um Loop Confiável**
    
    - 4.3.2.1. **Trigger (Gatilho):** O evento que inicia o ciclo — um agendamento (cron), um evento externo (PR aberto, teste que falhou, e-mail recebido) ou uma instrução humana pontual.
    - 4.3.2.2. **Goal Verificável (não uma aspiração vaga):** Um estado-alvo checável por máquina. Em vez de "melhorar a qualidade do código", especificar algo mensurável como "todos os testes passam" ou "bundle final abaixo de 200KB" — sem uma meta checável, o loop nunca sabe quando parar.
    - 4.3.2.3. **Verifier (Verificador):** O mecanismo que confirma se a meta foi atingida após cada iteração. Regra de ouro: **preferir verificação determinística** (código de saída, suíte de testes, linter) a pedir para o próprio modelo avaliar seu próprio trabalho — nunca deixar quem escreveu a solução julgar sozinho a própria qualidade.
    - 4.3.2.4. **Stop Rules (Regras de Parada):** Múltiplas saídas independentes para evitar loops infinitos — saída de sucesso (meta atingida), limite de iterações (iteration cap) e limite de orçamento (budget cap), para evitar gasto financeiro descontrolado em execuções autônomas.
- **4.3.3. Blocos de Construção de Loops em Produção**
    
    - 4.3.3.1. Loops maduros de produção adicionam cinco blocos além do ciclo básico agir-observar-decidir: **Automations** (descoberta de trabalho agendada), **Worktrees** (espaços de trabalho isolados por execução), **Skills** (conhecimento persistente do projeto entre execuções), **Plugins/Connectors** (integrações via MCP) e **Sub-agentes** (verificação independente do trabalho do agente principal).
    - 4.3.3.2. **Boas práticas:** começar pelo loop mais simples possível e só adicionar complexidade quando ela se mostrar necessária; escrever a meta como uma verificação executável antes de programar o agente; testar um único ciclo ação-verificação antes de adicionar lógica de retry; nunca rodar um loop sem supervisão sem as três regras de parada implementadas.

### 4.4 Graph Engineering — A Topologia da Orquestração Multi-Agente _(2026)_

> Se Loop Engineering definiu como construir um agente confiável até meados de 2026, Graph Engineering é a camada seguinte: a arte de conectar muitos loops de feedback em uma rede onde eles se observam, se restringem e se corrigem mutuamente.

- **4.4.1. Definição e Motivação**
    
    - 4.4.1.1. **O que é:** Graph Engineering trata a organização de um sistema multi-agente — papéis, dependências, caminhos de delegação e a estrutura de tarefas em tempo de execução — como um **grafo projetado explicitamente**, em vez de uma aglomeração acidental de chamadas de LLM que "dá certo na demo".
    - 4.4.1.2. **Não confundir com Knowledge Graph:** Graph Engineering, nesse contexto, não tem relação com grafos de conhecimento (estruturar dados/relações entre entidades) — trata-se de estruturar a **organização do próprio sistema de agentes**, não os dados que eles consomem (esse último já é coberto pelo GraphRAG, visto em 2.2.1.5).
- **4.4.2. Componentes de um Grafo de Agentes**
    
    - 4.4.2.1. **Nodes (Nós):** Agentes, funções determinísticas, roteadores, pontos de junção (joins), ferramentas e checkpoints humanos. Nem todo nó precisa ser um agente de IA — lógica de negócio determinística deve permanecer separada de tarefas semânticas.
    - 4.4.2.2. **Edges (Arestas):** Caminhos de controle de fluxo entre nós — conexões diretas, ramificações condicionais, caminhos paralelos, loops internos e handlers de erro. Uma aresta representa uma dependência ou regra de roteamento.
    - 4.4.2.3. **State (Estado Compartilhado):** Estrutura de dados tipada que flui pelo grafo. Um estado tipado torna visíveis as entradas e saídas de cada nó e reduz a necessidade de repassar a transcrição completa da conversa para cada agente.
    - 4.4.2.4. **Conditional Routing (Roteamento Condicional):** Condições de guarda que determinam qual aresta é executada a seguir — podem ser lógica determinística em código ou classificação via LLM. Regra central: restrições rígidas (*hard constraints*) não devem ficar escondidas dentro de prompts, e sim aplicadas explicitamente no código de roteamento.
- **4.4.3. Padrão Prático: Ciclo Avaliador-Otimizador (Evaluator-Optimizer)**
    
    - 4.4.3.1. Um fluxo de pesquisa típico em LangGraph ilustra o padrão: **Planner** (cria a estratégia) → **Researcher** (coleta evidências) → **Writer** (redige o rascunho) → **Evaluator** (avalia a qualidade) → roteamento condicional — se a nota de qualidade for menor que 0.8 **e** o número de revisões for menor que 2, volta para revisão; caso contrário, segue para **Human-in-the-Loop** (aprovação com interrupção) → **Finalize**. O limite de revisões fica explícito na estrutura do grafo, e não implícito em instruções de prompt — evitando loops infinitos de auto-correção.
- **4.4.4. Governança, Custo e Observabilidade em Produção**
    
    - 4.4.4.1. **Governança:** cada chamador governado independentemente precisa de identidade resolvida; chamadas de modelo e ferramentas mediadas por gateway precisam de política explícita definindo quais nós podem acessar quais recursos.
    - 4.4.4.2. **Controle de custo:** grafos de trabalho aumentam o número de chamadas ao modelo via *fan-out* e retries, exigindo orçamentos explícitos mapeados por conta virtual ou metadados, correlacionados por identificadores (`graph_id`, `run_id`, `node_id`) propagados em cada chamada.
    - 4.4.4.3. **Observabilidade:** o orquestrador registra a topologia e o estado de execução real (não apenas o desenho teórico do grafo); gateways correlacionam esses dados com custo, latência e métricas de política, permitindo atribuir consumo e desempenho a nós específicos.
- **4.4.5. Loop Engineering vs Graph Engineering: Camadas Complementares**
    
    - 4.4.5.1. As duas disciplinas são **complementares, não concorrentes**: Loop Engineering controla o ciclo de comportamento de **um** agente (observar → raciocinar → agir → verificar); Graph Engineering controla a **topologia entre múltiplos nós heterogêneos**.
    - 4.4.5.2. Resumo da relação: um grafo cujos nós têm loops mal projetados é "um organograma de funcionários pouco confiáveis"; um grafo cujos loops são excelentes mas cuja topologia é acidental é uma "falha de coordenação em escala". Produção exige as duas coisas ao mesmo tempo — loops excelentes operando dentro de uma estrutura organizacional intencional.

| Aspecto | Loop Engineering | Graph Engineering |
|---|---|---|
| Escopo | Um agente, um ciclo de ação-verificação-repetição | Múltiplos nós heterogêneos, topologia completa |
| Pergunta central | "Quando esse agente para de tentar?" | "Quem fala com quem, e sob quais regras?" |
| Unidade de design | Trigger, meta verificável, verificador, regras de parada | Nós, arestas, estado compartilhado, roteamento condicional |
| Ferramentas típicas | Harnesses de agente (Claude Code, Cursor), automations, sub-agentes | LangGraph, grafos cíclicos direcionados, orquestradores multi-agente |
| Falha típica sem essa disciplina | Loop infinito, gasto descontrolado, resultado não verificado | Organograma acidental ("vibe orchestration"), sem rastreabilidade de custo/decisão |
...
| **Orchestration Design** | Loop Engineering (trigger/verifier/stop rules), Graph Engineering (nodes/edges/state) |
